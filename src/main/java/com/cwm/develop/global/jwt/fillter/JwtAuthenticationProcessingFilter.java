package com.cwm.develop.global.jwt.fillter;

import com.cwm.develop.global.jwt.service.JwtService;
import com.cwm.develop.global.jwt.util.PasswordUtil;
    ...
    private void checkRefreshTokenAndReIssueAccessToken(HttpServletResponse response, String refreshToken) {
        userRepository.findByRefreshToken(refreshToken) //추출한 RefreshToken을 통해 여기서 유저를 찾는다.
                .ifPresent(user -> { //유저가 존재한다면
                    ...
                    jwtService.sendAccessAndRefreshToken(response, jwtService.createAccessToken(user.getEmail()), //jwtService.createAccessToken(user.getEmail())로 액세스 토큰 재발급
    ...
    public void checkAccessTokenAndAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        jwtService.extractAccessToken(request) //액세스 토큰 추출
                ...
                .ifPresent(accessToken ->jwtService.extractEmail(accessToken) //이메일 추출
                        .ifPresent(email -> userRepository.findByEmail(email) //해당 이메일로 유저를 찾는다.
